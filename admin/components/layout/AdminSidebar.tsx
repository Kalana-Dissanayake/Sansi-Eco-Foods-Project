...
export default function AdminSidebar({ pendingOrders = 0, userName }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#100e2b] border-t border-white/5 z-30 flex">
        {SECTIONS.flatMap((s) => s.items)
          .filter((item) => hasPermission(item.permission))
          .slice(0, 5) // Limit to 5 icons on mobile bottom bar
          .map((item) => {
            const active = isActive(item.href);
            const badge = getBadgeValue(item.badgeType);
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex-1 flex flex-col items-center justify-center py-2 text-[10px] font-semibold relative ${
              ...
              >
                ...
                {badge > 0 && (
                  <span className="absolute top-1 right-1/4 w-4 h-4 bg-amber-500 text-slate-900 text-[9px] font-black rounded-full flex items-center justify-center">
                    {badge > 9 ? '9+' : badge}
                  </span>
                )}
              </Link>
